...
let main argv =
    ...
    let SaveLightning(posns:string[],stss:stats[]) =
        ...
        for i = 0 to posns.Length-1 do
            let cd = tx.Put(db,Encoding.UTF8.GetBytes(posns.[i]),MessagePackSerializer.Serialize<stats>(stss.[i]))
            if int(cd)<>0 then 
    ...
    let cd = CreateLightning()
    let pstss = Array.zip posns stss
    st <- DateTime.Now
    let cd = SaveLightning(posns,stss)
    nd <- DateTime.Now
    logtime()
    st <- DateTime.Now
    let nstss = ReadLightning(posns.[..1000])
    nd <- DateTime.Now
    logtime()
    let sts = nstss.[99]
    ...
    st <- DateTime.Now
    let nstss = ReadLightning(posns.[1000..1000])
    nd <- DateTime.Now
    logtime()
    let env = new LightningDB.LightningEnvironment(lfol2)
    let newfol = Path.Combine(lfol,"compact") 
    Directory.CreateDirectory(newfol)|>ignore
    env.Open()
    env.CopyTo(newfol,true)
    env.Dispose()
