...
//
void PrintGameList(Index* i, NameBase* nb, gameNumberT first,
...
void printNameInfo(NameBase* nb, Index* idx)
{
    ...
    printf("Database %s: %u players, %u events, %u sites, %u rounds.\n",
        filename,
    ...
    for (nt = NAME_PLAYER; nt < NUM_NAME_TYPES; nt++)
    {
        ...
        idNumberT currentID = 0;
        uint numUnused = 0;

        nb->IterateStart(nt);
        for (uint i = 0; i < nb->GetNumNames(nt); i++)
        {
            idNumberT prevID = currentID;
        ...
        printf("\n%s section:    %u names, %u unused, %u disk bytes\n",
            ntStr[nt],
            nb->GetNumNames(nt),
            numUnused,
            nb->GetNumBytes(nt));
...
int main(int argc, char* argv[])
{
        ...
        option[1] == 's' || option[1] == 'r')
    {
        ...
        printf("[%u names matched the supplied prefix string]\n", matches);
    ...
    { // change description for database
    ...
    { // general information
        ...
        {
            printf(" (warning: version too old! This is Scid %d.%d).",
        ...
        nb->CloseNameFile();
        printf("\nNamebase: %u players, %u events, %u sites, %u rounds.\n",
    ...
    { // list all vital game information
        err = idx->OpenIndexFile(FMODE_ReadOnly);
        if (err != OK)
        {
            fileErr(filename, INDEX_SUFFIX, err);
    ...
    { // Compact and reorder the database
        compactGameFile();
