...
void NameBase::Init()
{
    ...
    MostFrequent[NUM_NAME_TYPES - 1] = {0};
}
...
NameBase::ReadNameFile(const char *suffix)
{
    ...
    ASSERT(ArraySize[NAME_PLAYER] == 0 && ArraySize[NAME_EVENT] == 0 && ArraySize[NAME_SITE] == 0 && ArraySize[NAME_ROUND] == 0);
    ...
    for (nameT nt = NAME_PLAYER; nt < NUM_NAME_TYPES; nt++)
    {
        ...
        for (idNumberT i = 0; i < Header.numNames[nt]; i++)
        {
            ...
            { // Frequencies all <= 255: fit in one byte
...
NameBase::IncArraySize(nameT nt, idNumberT increment)
{
    ...
    if (ArraySize[nt] > 0)
    { // copy old arrays into new then delete old
...
//
uint NameBase::GetFirstMatches(nameT nt, const char *str, uint maxMatches,
                               idNumberT *array)
{
...
//
uint NameBase::DumpAllNames(nameT nt, const char *prefixStr, FILE *f)
{
...
//
uint NameBase::TreeHeight(nameT nt)
{
    ASSERT(IsValidNameType(nt));
