...
dateT IndexEntry::GetEventDate(void)
{
...
IndexEntry::Read(MFile *fp, versionT version)
{
    ...
    if (GetWhiteElo() > MAX_ELO)
    {
        SetWhiteElo(MAX_ELO);
    }
    if (GetBlackElo() > MAX_ELO)
    {
        SetBlackElo(MAX_ELO);
    }
...
uint IndexEntry::CharToFlag(char ch)
{
    ...
    {
    case 'D':
        flag = IDX_FLAG_DELETE;
        break;
    case 'W':
        flag = IDX_FLAG_WHITE_OP;
        break;
    case 'B':
        flag = IDX_FLAG_BLACK_OP;
        break;
    case 'M':
        flag = IDX_FLAG_MIDDLEGAME;
        break;
    case 'E':
        flag = IDX_FLAG_ENDGAME;
        break;
    case 'N':
        flag = IDX_FLAG_NOVELTY;
        break;
    case 'P':
        flag = IDX_FLAG_PAWN;
        break;
    case 'T':
        flag = IDX_FLAG_TACTICS;
        break;
    case 'K':
        flag = IDX_FLAG_KSIDE;
        break;
    case 'Q':
        ...
        break;
    case '!':
        flag = IDX_FLAG_BRILLIANCY;
        break;
    case '?':
        flag = IDX_FLAG_BLUNDER;
        break;
    case 'U':
        flag = IDX_FLAG_USER;
        break;
    case '1':
        flag = IDX_FLAG_CUSTOM1;
        break;
    case '2':
        flag = IDX_FLAG_CUSTOM2;
        break;
    case '3':
        flag = IDX_FLAG_CUSTOM3;
        break;
    case '4':
        flag = IDX_FLAG_CUSTOM4;
        break;
    case '5':
        flag = IDX_FLAG_CUSTOM5;
        break;
    case '6':
        flag = IDX_FLAG_CUSTOM6;
        break;
    }
    return flag;
}
...
void IndexEntry::SetFlagStr(const char *flags)
{
...
uint IndexEntry::GetFlagStr(char *str, const char *flags)
{
    if (flags == NULL)
    {
        flags = "DWBMENPTKQ!?U123456";
    }
    uint count = 0;
    while (*flags != 0)
    {
...
//
void IndexEntry::PrintGameInfo(char *outStr,
                               ...
                               NameBase *nb, const char *format)
{
    ...
    *out = 0;
    ...
    while (*s)
    {
        ...
        { // Saw a letter, so it is a field type:
            char c = *s++;
            int width = 0;
            ...
            {
                width = 1;
            } // Minimum fixed width = 1.

            switch (c)
            {
            case 'f': // Game number, filtered (Currently unused)
                sprintf(temp, "%*u", width, gnFiltered);
                out = strAppend(out, temp);
                break;

            case 'g': // Game Number
                sprintf(temp, "%*u", width, gnReal);
                out = strAppend(out, temp);
                break;

            case 'G': // Game file offset
                sprintf(temp, "%*u", width, Offset);
                out = strAppend(out, temp);
                break;

            case 'L': // Game file record length
                sprintf(temp, "%*u", width, GetLength());
                out = strAppend(out, temp);
                break;
            ...
            case 'y': // Year
                sprintf(temp, "%4u", date_GetYear(GetDate()));
                out = strAppend(out, temp);
                break;
...
//
int IndexEntry::Compare(IndexEntry *ie, int *fields, NameBase *nb)
{
    int res, rOne, rTwo;
...
Index::Open(fileModeT fmode, bool old)
{
    ...
    if (!old)
    {
        if (Header.version > SCID_VERSION)
        {
            result = ERROR_OldScidVersion;
        }
...
//
uint Index::VerifyFile(NameBase *nb)
{
    ASSERT(FilePtr != NULL);
    if (!InMemory)
    {
...
{
    ASSERT(FilePtr != NULL && (start + count) <= Header.numGames);
...
//
void Index::SetDescription(const char *str)
{
    ASSERT(str != NULL);
    uint i;
    const char *s = str;
...
Index::AddGame(gameNumberT *g, IndexEntry *ie, bool initIE)
{
    ...
    if (Header.numGames >= MAX_GAMES)
    {
        return ERROR_IndexFull;
    }
...
//
void Index::Sort_AdjustHeap(int heapSize, int root, NameBase *nb)
{
    ...
    {
        if (FetchEntry(EntriesHeap[current])->Compare(FetchEntry(EntriesHeap[current + 1]), SortCriteria, nb) < 0)
        {
