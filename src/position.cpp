...
Position::AssertPos()
{
    ...
    for (colorT c = WHITE; c <= BLACK; c++)
    {
        ...
        {
            if (ListPos[List[c][i]] != i || piece_Color(Board[List[c][i]]) != c)
            {
                DumpBoard(stderr);
                DumpLists(stderr);
                return ERROR;
...
void Position::GenCastling(MoveList *mlist)
{
    ...
    if (!StrictCastling || GetCastling(ToMove, KSIDE))
    {
        ...
        if (Board[target] == EMPTY && Board[skip] == EMPTY && Board[rookSq] == rookPiece && CalcNumChecks(target) == 0 && CalcNumChecks(skip) == 0 && !square_Adjacent(target, enemyKingSq))
        {
    ...
    if (!StrictCastling || GetCastling(ToMove, QSIDE))
    {
        ...
        if (Board[target] == EMPTY && Board[skip] == EMPTY && Board[rookSq] == rookPiece && Board[target - 1] == EMPTY // B1 or B8 must be empty too!
            && CalcNumChecks(target) == 0 && CalcNumChecks(skip) == 0 && !square_Adjacent(target, enemyKingSq))
        {
            AddLegalMove(mlist, from, target, EMPTY);
        }
    }
...
//
void Position::GenKingMoves(MoveList *mlist, genMovesT genType, bool castling)
{
    ...
    while (*destPtr != NULL_SQUARE)
    {
        ...
        if ((genNonCaptures && Board[destSq] == EMPTY) || piece_Color(Board[destSq]) == enemy)
        {
...
void Position::GenPawnMoves(MoveList *mlist, squareT from,
                            directionT dir, SquareSet *sqset, genMovesT genType)
{
    ...
    // an enemy piece.
#define POSSIBLE_CAPTURE(d) ((d != NULL_SQUARE) && ((piece_Color(Board[d]) == (color_Flip(ToMove))) || (d == EPTarget && IsValidEnPassant(from, d))))
...
//      If sqset != NULL, moves must be to a square in sqset.<
void Position::GenPieceMoves(MoveList *mlist, squareT fromSq,
                             SquareSet *sqset, bool capturesOnly)
{
    colorT c = ToMove;
                             ...
                             genMovesT genType, bool maybeInCheck)
{
    ...
    {
        mask = (1 << KING) | (1 << QUEEN) | (1 << ROOK) | (1 << BISHOP) | (1 << KNIGHT) | (1 << PAWN);
    }
    ...
    for (uint x = 1; x < npieces; x++)
    {
        ...
        { // piece x is pinned to king
            ...
            else
            {
                ...
                if (ptype == QUEEN || (ptype == ROOK && !direction_IsDiagonal(pinned)) || (ptype == BISHOP && direction_IsDiagonal(pinned)))
                {
...
//
void Position::MatchLegalMove(MoveList *mlist, pieceT mask, squareT target)
{
    uint x;
...
Position::MatchKingMove(MoveList *mlist, squareT target)
{
    ...
    { // KingSide Castling
        ...
        if (Board[kingSq + 1] != EMPTY || Board[kingSq + 2] != EMPTY || CalcNumChecks(kingSq) > 0 || CalcNumChecks(kingSq + 1) > 0 || CalcNumChecks(kingSq + 2) > 0)
        {
            return ERROR_InvalidMove;
        }
    ...
    { // Queenside Castling
        ...
        if (Board[kingSq - 1] != EMPTY || Board[kingSq - 2] != EMPTY || Board[kingSq - 3] != EMPTY || CalcNumChecks(kingSq) > 0 || CalcNumChecks(kingSq - 1) > 0 || CalcNumChecks(kingSq - 2) > 0)
        {
            return ERROR_InvalidMove;
        }
        AddLegalMove(mlist, kingSq, target, EMPTY);
...
uint Position::CalcAttacks(colorT side, squareT target, SquareList *fromSquares)
{
    ...
    if (Material[knight] > 0 && SquareColorCount(knight, color_Flip(square_Color(target))) > 0)
    {
...
bool Position::IsStaleMate(void)
{
    SquareList checkSquares;
    uint numChecks = CalcNumChecks(GetKingSquare(ToMove), &checkSquares);
    if (numChecks > 0)
...
bool Position::IsPromoMove(squareT from, squareT to)
{
...
void Position::DoSimpleMove(simpleMoveT *sm)
{
    ...
    if (ptype == PAWN && sm->capturedPiece == EMPTY && square_Fyle(from) != square_Fyle(to))
    {
    ...
    if (ptype == PAWN)
    {
        ...
        rankT toRank = square_Rank(to);
        if (fromRank == RANK_2 && toRank == RANK_4 && (Board[square_Move(to, LEFT)] == BP || Board[square_Move(to, RIGHT)] == BP))
        {
            EPTarget = square_Move(from, UP);
        }
        if (fromRank == RANK_7 && toRank == RANK_5 && (Board[square_Move(to, LEFT)] == WP || Board[square_Move(to, RIGHT)] == WP))
        {
...
void Position::UndoSimpleMove(simpleMoveT *m)
{
    ...
    if ((piece_Type(p) == KING) && square_Fyle(from) == E_FYLE && (square_Fyle(to) == C_FYLE || square_Fyle(to) == G_FYLE))
    {
        squareT rookfrom, rookto;
...
void Position::MakeSANString(simpleMoveT *m, char *s, sanFlagT flag)
{
    ...
    if (p == PAWN)
    {
        if (square_Fyle(from) != square_Fyle(to))
        { // pawn capture
...
//
void Position::MakeUCIString(simpleMoveT *m, char *s)
{
    ASSERT(m != NULL && s != NULL);
    ...
    *s2 = '\0';
    ...
    if (token == TOKEN_Move_Pawn || token == TOKEN_Move_Promote)
    {
        ...
        if (token == TOKEN_Move_Promote)
        {
            ...
            if (promo != QUEEN && promo != ROOK && promo != KNIGHT && promo != BISHOP)
            {
                return ERROR_InvalidMove;
            }
            slen--;
    ...
    if (token != TOKEN_Move_Piece)
    { // Must be castling move
        ASSERT(token == TOKEN_Move_Castle_King || token == TOKEN_Move_Castle_Queen);
        squareT from = (ToMove == WHITE ? E1 : E8);
    ...
    for (i = 0; i < mlist.Size(); i++)
    {
        ...
        if (to == thisMove->to && (frFyle == NO_FYLE || frFyle == square_Fyle(thisMove->from)) && (frRank == NO_RANK || frRank == square_Rank(thisMove->from)))
        {
            // We have a match!!
            *m = *thisMove;
            matchCount++;
...
Position::ReadLine(const char *line)
{
    ...
    while (1)
    {
        ...
        *s2 = '\0';
        ...
        errorT err = ReadMove(&sm, mStr, token);
        if (err != OK)
        {
            return err;
        }
...
//
void Position::CalcSANStrings(sanListT *sanList, sanFlagT flag)
{
...
void Position::CalcUCIStrings(sanListT *sanList)
{
    if (LegalMoves.Size() == 0)
    {
        GenerateMoves();
...
//
void Position::PrintFEN(char *str, uint flags)
{
    ASSERT(str != NULL);
    uint iRank, iFyle;
    for (iRank = 0; iRank < 8; iRank++)
    {
        pieceT *pBoard = &(Board[(7 - iRank) * 8]);
        uint emptyRun = 0;
...
void Position::DumpHtmlBoard(DString *dstr, uint style, const char *dir, bool flip)
{
...
void Position::DumpLatexBoard(DString *dstr, bool flip)
{
    uint iRank, iFyle;
    dstr->Append("\\board{");
    for (iRank = 0; iRank < 8; iRank++)
