...
void PBook::SetHashFlag(Position *pos)
{
    ...
    if (HashFlags == NULL)
    {
        ...
        for (uint i = 0; i<(PBOOK_HASH_BYTES>> 3); i++)
        {
            HashFlags[i] = 0;
        }
    }
...
bool PBook::GetHashFlag(Position *pos)
{
    uint hash = PBOOK_HASH(pos);
    uint index = hash >> 3;
    uint mask = 1 << (hash & 7);
...
void PBook::AddNodeToList(bookNodeT *node)
{
...
void PBook::SetFileName(const char *fname)
{
    if (FileName)
    {
        delete[] FileName;
...
PBook::Insert(Position *pos, const char *comment)
{
    ...
    if (err != OK)
    { // Already exists; we overwrite the old data.
        delete[] node->data.comment;
    }
    else
    {
...
void PBook::EcoSummary(const char *ecoPrefix, DString *dstr)
{
